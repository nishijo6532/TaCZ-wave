...
public class GunAnimationStateContext extends ItemAnimationStateContext {
    ...
    private float compatWalkDist = 0f;
    private float compatWalkDistO = 0f;
    private int compatWalkDistTick = Integer.MIN_VALUE;
    private int compatWalkDistEntityId = Integer.MIN_VALUE;
    ...
    private float getCompatWalkDist(LivingEntity livingEntity) {
        if (compatWalkDistEntityId != livingEntity.getId()) {
            compatWalkDist = 0f;
            compatWalkDistO = 0f;
            compatWalkDistTick = Integer.MIN_VALUE;
            compatWalkDistEntityId = livingEntity.getId();
        }
        if (compatWalkDistTick != livingEntity.tickCount) {
            compatWalkDistO = compatWalkDist;
            compatWalkDist += (float) (livingEntity.getDeltaMovement().horizontalDistance() * 0.6D);
            compatWalkDistTick = livingEntity.tickCount;
        }
        return compatWalkDist + (compatWalkDist - compatWalkDistO) * partialTicks;
    }
    ...
    public void adjustClientShootInterval(long alpha) {
        processGunOperator(operator -> {
            if (iGun == null || currentGunItem == null) {
                return null;
            }
            var holder = operator.getDataHolder();
            Identifier currentGunId = iGun.getGunId(currentGunItem);
            if (!currentGunId.equals(holder.clientShootGunId)) {
                return null;
            }
    ...
    public void anchorWalkDist() {
        processCameraEntity(entity -> {
            if (entity instanceof LivingEntity livingEntity) {
                walkDistAnchor = getCompatWalkDist(livingEntity);
            }
            return null;
        });
    }
    ...
    public float getWalkDist() {
        return processCameraEntity(entity -> {
            float currentWalkDist = entity instanceof LivingEntity livingEntity ?
                    getCompatWalkDist(livingEntity) : 0f;
    ...
    public void setCurrentGunItem(ItemStack currentGunItem) {
        ...
        if (NbtCompat.hasTag(currentGunItem)) {
            nbtUtil = new LuaNbtAccessor(NbtCompat.getTag(currentGunItem));
        }
    }
}
