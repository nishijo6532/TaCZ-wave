...
public class InteractKey {
    public static final KeyMapping INTERACT_KEY = KeyMappingCompat.create("key.tacz.interact.desc",
    ...
    public static void onInteractKeyPress(InputEvent.Key event) {
        if (isInGame() && event.getAction() == GLFW.GLFW_PRESS && KeyMappingCompat.matches(INTERACT_KEY, event)) {
            doInteractLogic();
        }
    }
    ...
    public static void onInteractMousePress(InputEvent.MouseButton.Post event) {
        if (isInGame() && event.getAction() == GLFW.GLFW_PRESS && KeyMappingCompat.matchesMouse(INTERACT_KEY, event)) {
            doInteractLogic();
        }
    }
    ...
    private static void startUseItem(Minecraft mc) {
        try {
            Method method = Minecraft.class.getDeclaredMethod("startUseItem");
