...
public class ZoomKey {
    public static final KeyMapping ZOOM_KEY = KeyMappingCompat.create("key.tacz.zoom.desc",
    ...
    public static void onZoomKeyPress(InputEvent.Key event) {
        if (isInGame() && event.getAction() == GLFW.GLFW_PRESS && KeyMappingCompat.matches(ZOOM_KEY, event)) {
            doZoomLogic();
        }
    }
    ...
    public static void onZoomMousePress(InputEvent.MouseButton.Post event) {
        if (isInGame() && event.getAction() == GLFW.GLFW_PRESS && KeyMappingCompat.matchesMouse(ZOOM_KEY, event)) {
            doZoomLogic();
        }
    }
