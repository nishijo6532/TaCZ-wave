...
public class ClientAttachmentItemTooltip implements ClientTooltipComponent {
    ...
    public void extractText(GuiGraphicsExtractor gui, Font font, int pX, int pY) {
        ...
        for (Component component : this.components) {
            gui.text(font, component, pX, yOffset, 0xFFFFAA00, false);
            yOffset += 10;
        }
        if (!isShiftDown()) {
            gui.text(font, tips, pX, pY + 5 + this.components.size() * 10, 0xFF9E9E9E, false);
            yOffset += 10;
        } else {
            yOffset += (showGuns.size() - 1) / 16 * 18 + 32;
        }
        if (packInfo != null) {
            gui.text(font, this.packInfo, pX, yOffset + 8, 0xFFFFFFFF, false);
        }
    }
    ...
    public void extractImage(Font font, int mouseX, int mouseY, int width, int height, GuiGraphicsExtractor gui) {
        ...
        gui.text(font, support, mouseX + 2, mouseY + minY + 2, 0xFFE3F2FD);
