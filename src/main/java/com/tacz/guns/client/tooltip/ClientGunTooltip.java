...
public class ClientGunTooltip implements ClientTooltipComponent {
    ...
    public void extractText(GuiGraphicsExtractor gui, Font font, int pX, int pY) {
        ...
        if (shouldShow(GunTooltipPart.DESCRIPTION) && this.desc != null) {
            yOffset += 2;
            for (FormattedCharSequence sequence : this.desc) {
                gui.text(font, sequence, pX, yOffset, 0xFFAAAAAA, false);
                yOffset += 10;
            }
        }
        ...
        if (shouldShow(GunTooltipPart.AMMO_INFO)) {
            yOffset += 4;
            gui.text(font, this.ammoName, pX + 20, yOffset, 0xFFFFAA00, false);
            gui.text(font, this.ammoCountText, pX + 20, yOffset + 10, 0xFF777777, false);
            yOffset += 20;
        }

        if (shouldShow(GunTooltipPart.BASE_INFO)) {
            yOffset += 4;
            gui.text(font, this.levelInfo, pX, yOffset, 0xFF777777, false);
            yOffset += 10;

            if (this.gunType != null) {
                gui.text(font, this.gunType, pX, yOffset, 0xFF777777, false);
                yOffset += 10;
            }

            gui.text(font, this.damage, pX, yOffset, 0xFF777777, false);
            yOffset += 10;
        }

        if (shouldShow(GunTooltipPart.EXTRA_DAMAGE_INFO)) {
            yOffset += 4;
            gui.text(font, this.armorIgnore, pX, yOffset, 0xFFFFAA00, false);
            yOffset += 10;

            gui.text(font, this.headShotMultiplier, pX, yOffset, 0xFFFFAA00, false);
            yOffset += 10;

            gui.text(font, this.weight, pX, yOffset, 0xFFFFFFFF, false);
            yOffset += 10;
        }

        if (shouldShow(GunTooltipPart.UPGRADES_TIP)) {
            yOffset += 4;
            gui.text(font, this.tips, pX, yOffset, 0xFFFFFFFF, false);
            yOffset += 10;
        }

        if (shouldShow(GunTooltipPart.PACK_INFO) && packInfo != null) {
            yOffset += 4;
            gui.text(font, this.packInfo, pX, yOffset, 0xFFFFFFFF, false);
        }
    }
