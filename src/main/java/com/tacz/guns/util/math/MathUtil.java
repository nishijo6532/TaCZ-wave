...
public class MathUtil {
    ...
    public static void normalizeQuaternion(Quaternionf q) {
        ...
        if (f > 0) {
            float f1 = 1.0F / (float) Math.sqrt(f);
    ...
    public static Quaternionf multiplyQuaternion(Quaternionf quaternion, float multiplier) {
        Quaternionf normalized = new Quaternionf(quaternion);
        normalizeQuaternion(normalized);
        Pair<Float, Vector3f> angleAndAxis = getAngleAndAxis(normalized);
        if (angleAndAxis.getRight().lengthSquared() < 1.0E-8F) {
        ...
        axis.mul((float) sin);
        Quaternionf result = new Quaternionf(axis.x(), axis.y(), axis.z(), (float) cos);
        normalizeQuaternion(result);
        return result;
    }

    public static float[] multiplyQuaternion(float[] quaternion, float multiplier) {
        Quaternionf normalized = toQuaternion(quaternion);
        normalizeQuaternion(normalized);
        Pair<Float, Vector3f> angleAndAxis = getAngleAndAxis(normalized);
        if (angleAndAxis.getRight().lengthSquared() < 1.0E-8F) {
